...
export default function ManagePage() {
  ...
  // One-time migration: bookings saved before the multi-event update had
  // a single `date`/`venue`/`crewIds` directly on the booking. Convert
  // any of those to the new `events[]` shape so existing data survives.
  useEffect(() => {
    if (!bookingsHydrated) return;
    const needsMigration = bookings.some((b) => !Array.isArray((b as unknown as { events?: unknown }).events));
    if (needsMigration) {
      setBookings(bookings.map(migrateBooking));
    }
